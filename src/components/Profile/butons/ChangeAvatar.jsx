...
const ChangeAvatar = ({ token, setUserInfo, handleReloadProfile, closeModal }) => { // Truyền hàm closeModal từ component cha vào
    ...
    useEffect(() => {
        const handleEsc = (event) => {
            if (event.keyCode === 27) { // Xử lý sự kiện nhấn phím Esc
                closeModal();
            }
        };
        window.addEventListener('keydown', handleEsc); // Thêm event listener khi component được mount
        return () => {
            window.removeEventListener('keydown', handleEsc); // Xóa event listener khi component unmount
        };
    }, [closeModal]);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // Cập nhật đường dẫn ảnh avatar mới vào state userInfo
            setUserInfo(userInfo => ({
                ...userInfo,
                Avatar: response.data.newAvatarUrl
            }));
    ...
    return (
        <>
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div className="max-w-md mx-auto bg-white p-8 rounded-md relative">
                    ...
                    {imagePreview && <img src={imagePreview} alt="Selected Avatar" className="w-full h-auto mb-4" />}
